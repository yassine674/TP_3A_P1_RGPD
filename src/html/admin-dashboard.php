...
// Traiter les actions (supprimer, changer statut)
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['delete_id'])) {
        ...
        $stmt = $conn->prepare("SELECT cv_path FROM candidatures WHERE id = ?");
        $stmt->bind_param("i", $delete_id);
        $stmt->execute();
        $result = $stmt->get_result();
        if ($row = $result->fetch_assoc()) {
            // Supprimer le fichier CV
            if (!empty($row['cv_path']) && file_exists($row['cv_path'])) {
                unlink($row['cv_path']);
            }
        }
        $stmt->close();
        ...
        // Log de l'activité
        $stmt = $conn->prepare("INSERT INTO activity_logs (user_id, action, details, ip_address) VALUES (?, 'DELETE', ?, ?)");
        $details = "Suppression candidature ID: " . $delete_id;
    ...
    // Changer le statut
    if (isset($_POST['change_status']) && isset($_POST['status_id']) && isset($_POST['new_status'])) {
        ...
        // Log
        $stmt = $conn->prepare("INSERT INTO activity_logs (user_id, action, details, ip_address) VALUES (?, 'UPDATE_STATUS', ?, ?)");
        $details = "Changement statut ID: " . $status_id . " vers " . $new_status;
        $ip = $_SERVER['REMOTE_ADDR'];
        $stmt->bind_param("iss", $_SESSION['user_id'], $details, $ip);
        $stmt->execute();
        $stmt->close();
...
<head>
    ...
    <title>Dashboard Admin - MEDICONNECT</title>
...
<body>
    ...
    <div class="container">
        ...
        <p class="page-subtitle">Gestion complète des candidatures</p>
                            ...
                            <tr>
                                ...
                                <td>
                                    <form method="POST" style="display: inline;">
                                        ...
                                        <select name="new_status" onchange="this.form.submit()">
                                            ...
                                            <option value="accepte" <?php echo $row['statut'] == 'accepte' ? 'selected' : ''; ?>>Acceptée</option>
                                            <option value="refuse" <?php echo $row['statut'] == 'refuse' ? 'selected' : ''; ?>>Refusée</option>
                                        </select>
                                ...
                                <td>
                                    <div class="action-buttons">
                                        ...
                                        <?php endif; ?>
                                        <form method="POST" style="display: inline;" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer cette candidature ?');">
